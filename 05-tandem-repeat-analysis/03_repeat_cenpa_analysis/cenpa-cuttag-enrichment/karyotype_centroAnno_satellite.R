# ============================================================================
# Genome-wide CENP-A signal + centroAnno satellite arrays
#
#   Blue bars:
...
#
#   Green ticks:
#       Midpoint locations of centroAnno satellite arrays (merged monomers > 1 kb).
#
#   Layout:
...
CENTROANNO_HOR_BED <- paste0(
  "/tscc/projects/ps-renlab2/jhc103/degu-genome-assembly-proj/",
  "figure/cenpa-cuttag-enrichment/data/foregrounds/setA_centroAnno_strict.bed"
)
...
for (chrom_i in plot_chromosomes) {
  cat(sprintf(
    "%s: %s bp\n", chrom_i,
    format(chr_lengths[chrom_i], big.mark = ",")
  ))
}
...
for (chrom_i in plot_chromosomes) {
  ...
  if (nrow(chr_bins) >= PEAK_WINDOW_BINS) {
    ...
    cat(sprintf(
      "Peak CENP-A (%s): midpoint = %s, score = %.1f\n",
      chrom_i,
...
# Columns: chrom, name, start, end, n_monomers, hor_length, span_length
# ----------------------------------------------------------------------------
...
# Use HOR midpoints as tick positions (BED is 0-based, adjust start)
hor_dt <- hor_raw[, .(
  chrom,
  start = start + 1L,
  end,
  midpoint = as.integer(floor((start + 1L + end) / 2))
)]
...
cat(sprintf(
  "centroAnno satellite arrays loaded: %d regions across %d chromosomes\n",
  nrow(hor_dt),
...
HOR_TICK_COLOR   <- "#238B45"   # Green ticks (centroAnno HOR arrays)
...
HOR_TICK_BOTTOM_OFFSET_IN <- 0.135
HOR_TICK_TOP_OFFSET_IN    <- 0.045
HOR_TICK_LWD              <- 0.70
...
# chr26-chr28 rows occupy roughly rows 26-28 (indices)
# Place legend in the space beside them
legend_anchor_row <- row_positions[25]  # beside chr25

LEGEND_SYMBOL_X <- 0.370
LEGEND_TEXT_X   <- 0.395

LEGEND_ITEM_GAP_IN <- 0.32
...
LEGEND_ROW_3_Y <- LEGEND_CENTER_Y - LEGEND_ITEM_GAP

# ----------------------------------------------------------------------------
# Title and output filenames
# ----------------------------------------------------------------------------

MAIN_TITLE <- "Genome-wide CENP-A signal and centroAnno satellite arrays"
...
OUTPUT_PDF <- file.path(
  FIGURE_DIR,
  "genome_wide_CENPA_centroAnno_satellite.pdf"
)

OUTPUT_SVG <- file.path(
  FIGURE_DIR,
  "genome_wide_CENPA_centroAnno_satellite.svg"
)
...
# ----------------------------------------------------------------------------
# Floating legend
# Triangle first, CENP-A second, HORs last
# ----------------------------------------------------------------------------
...
draw_floating_legend <- function() {
  ...
  text(
    ...
    y       = LEGEND_ROW_3_Y,
    labels  = sprintf(
      "centroAnno satellite regions (>1 kb merged monomers; n = %s)",
...
cat("\nDrawing genome-wide CENP-A + centroAnno satellite overview ...\n")
